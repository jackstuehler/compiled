...
export async function loadMajorData(cipCode: string) {
	...
	const [majors, schools] = await Promise.all([loadMajors(), loadSchools(cipCode)]);

	const current = majors.find((candidate) => candidate.cip_code === cipCode);
	let majorName = "Unknown major";
	if (current !== undefined) {
		majorName = current.cip_desc;
	}

	return { majors, schools, majorName };
}
