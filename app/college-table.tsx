...
import { CONTROL, DASH, MEASURES, collegeName, matchesSearch, measure, money, num, pct, place, searchableText, tokenize, type School } from "@/lib/measures";

/**
 * The official name, for the name cell's tooltip.
 *
 * Returns undefined when the official name is already what's on screen, which
 * leaves the title attribute off the cell entirely — there's no point in a
 * tooltip that repeats the text underneath it.
 */
function officialNameTooltip(school: School): string | undefined {
	if (collegeName(school) === school.instnm) {
		return undefined;
	}
...
export default function CollegeTable({ rows }: { rows: School[] }) {
	...
	const orderHint = isAscending ? "Ascending — click for descending" : "Descending — click for ascending";
		...
		<div className={styles.tableSection}>
			<div className={styles.controls}>
				<label className={ui.field}>
					Search
					<input className={ui.input} type="search" value={query} placeholder="e.g. MIT or Tufts" onChange={handleQueryChange} />
				</label>
				...
				<div className={styles.sortGroup}>
					<label className={ui.field}>
						Sort by
						<select className={ui.input} value={sortKey} onChange={handleSortKeyChange}>
							{MEASURES.map((option) => (
					...
					<button type="button" className={`${ui.button} ${styles.orderButton}`} onClick={toggleOrder} aria-label={orderLabel} title={orderHint}>
						{orderArrow}
					</button>
				</div>
			</div>
					...
					<tbody>
						{visibleRows.map((school, index) => (
							<tr key={school.unitid}>
								<td className={styles.firstColumn} title={officialNameTooltip(school)}>
                      ...
                      now, so it renumbers whenever the sort or filters change. */}
								...
								<td className={styles.numeric}>{money(school.grad_debt_mdn)}</td>
								<td className={styles.numeric}>{money(school.debt_all_stgp_eval_mdn)}</td>
							</tr>
						))}
					</tbody>
				</table>
